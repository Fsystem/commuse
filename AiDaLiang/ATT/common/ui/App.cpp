...
//LRESULT jkBaseDialog::OnSysCommand( WPARAM w,LPARAM l )
//{
//	return 1L;
//}
...
LRESULT jkBaseWindow::OnCommand( WPARAM w,LPARAM l )
{
	return 0L;
}

// LRESULT jkBaseWindow::OnSysCommand( WPARAM w,LPARAM l )
// {
// 	return 1L;
// }
...
void jkControl::SetStyle( DWORD dwStyle )
{
	mStyle &= ~dwStyle;
	mStyle |= dwStyle;
	if(IsWindow(mHwnd)) SetWindowLong(mHwnd,GWL_STYLE,GetWindowLong(mHwnd,GWL_STYLE)|dwStyle);
	UpdateWindow(mHwnd);
}
...
void jkControl::SetText( LPCSTR lpTitle )
{
	if (IsWindow(mHwnd))
	{
		SetWindowTextA(mHwnd,lpTitle);
	}
}

std::string jkControl::GetText()
{
	char szText[MAX_PATH];
	memset(szText,0,sizeof szText);
	if (IsWindow(mHwnd))
	{
		GetWindowTextA(mHwnd,szText,MAX_PATH);
	}
