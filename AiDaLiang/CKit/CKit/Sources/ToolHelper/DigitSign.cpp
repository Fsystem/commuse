...
bool GetSignner(HANDLE hWVTStateData, char* strSignner)
{
	PCRYPT_PROVIDER_DATA provider = NULL;
	PCRYPT_PROVIDER_SGNR signner = NULL;
	PCRYPT_PROVIDER_CERT cert = NULL;
	DWORD i = 0;
	if( NULL == hWVTStateData ||INVALID_HANDLE_VALUE == hWVTStateData)
	{
		return false;
	}
	...
	i = CertGetNameString(cert->pCert, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, NULL, 0);
	char strName[MAX_PATH]={0};
	CertGetNameStringA(cert->pCert, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, NULL, strName, MAX_PATH);
	strncpy(strSignner,strName,MAX_PATH);
	return true;
}
//C++ 验证微软数字签名   
BOOL CheckFileTrust_ex( LPCSTR lpFileName, char* strCompany)
{
	if( NULL == lpFileName)
	{
		return FALSE;
	...
	BOOL bRet = FALSE;
	WINTRUST_DATA WTrustData = { 0 };
	WINTRUST_FILE_INFO WTDFileInfo = { 0 };
	WINTRUST_CATALOG_INFO WTDCatalogInfo = { 0 };
	...
	if (mbstowcs(szFileName, lpFileName, MAX_PATH) == -1)
	{
		DWORD dwErr = ::GetLastError();
		LOGEVEN(TEXT("error:mbstowcs failed[%s][err=%u]\n") , lpFileName, dwErr);
		return FALSE;
	}
	...
	if ( NULL == hCatInfo )
	{
		WTDFileInfo.cbStruct       = sizeof( WINTRUST_FILE_INFO );
		WTDFileInfo.pcwszFilePath  = szFileName;
		WTDFileInfo.hFile          = NULL;
		WTDFileInfo.pgKnownSubject = NULL;

		WTrustData.cbStruct = sizeof(WINTRUST_DATA);
		WTrustData.dwUnionChoice = WTD_CHOICE_FILE;
		WTrustData.pFile           = &WTDFileInfo;
	}
	else
	{
		CryptCATCatalogInfoFromContext( hCatInfo, &CatalogInfo, 0 );
		WTDCatalogInfo.cbStruct = sizeof(WTDCatalogInfo);
		WTDCatalogInfo.pcwszCatalogFilePath = CatalogInfo.wszCatalogFile;
		WTDCatalogInfo.pcwszMemberFilePath = szFileName;
		WTDCatalogInfo.pcwszMemberTag = pszMemberTag;
		WTrustData.cbStruct = sizeof(WTrustData);
		WTrustData.dwUnionChoice = WTD_CHOICE_CATALOG;
		WTrustData.pCatalog = &WTDCatalogInfo;
	}
	GUID action = WINTRUST_ACTION_GENERIC_VERIFY_V2;
	HRESULT hr  = WinVerifyTrust( NULL, &action, &WTrustData );
	bRet        = SUCCEEDED( hr );
	if(!bRet)
	{
	...
	if(bRet)
	{
		if( NULL != WTrustData.hWVTStateData)
		{
			GetSignner(WTrustData.hWVTStateData, strCompany);

		}
		WTrustData.dwStateAction = WTD_STATEACTION_CLOSE;
		hr  = WinVerifyTrust( NULL, &action, &WTrustData );
	}
	if ( NULL != hCatInfo )
	{
...
BOOL CDigitSign::CheckFileTrust(LPCSTR filename,LPSTR pszCompany)
{
	BOOL bRet = FALSE;
	char szCompany[MAX_PATH]={0};
	memset(szCompany,0,MAX_PATH);

	PVOID old_value = NULL;
	if(GetWOW64PlatformSupport()) SetDisableOrEnable(FALSE,&old_value);

	locale loc = locale::global(gLocale);
	bRet = CheckFileTrust_ex(filename,szCompany);
	locale::global(loc);

	if(GetWOW64PlatformSupport()) SetDisableOrEnable(TRUE,&old_value);

	strncpy(pszCompany,szCompany,_countof(szCompany));
