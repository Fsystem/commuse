...
namespace COMMUSE
{
	...
	template<class BaseType, class StringContentType>
	void SplitString(const BaseType* pszSource, const BaseType* pszFlags, StringContentType& strReturn)
	{
		strReturn.clear();
		...
		if (iFlagLen == 0)
		{
			StringContentType::value_type str(pszSource, iSourceLen);
			...
			pPosFind = SplitAssist<BaseType>::FindString(pPosStart, pszFlags))
		{
			...
			if (iLenValid > 0)
			{
				StringContentType::value_type str(pPosStart, iLenValid);
				strReturn.push_back(str);
			}
