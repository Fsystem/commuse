...
int ns__paynotify(struct soap *pWebSoap,  char* szAccount ,int nKindID,int nServerID, int& nRes)
{
	nRes=0;
	if(PROCESS_WEB_INSTANCE.GetSink())
	{
		PROCESS_WEB_INSTANCE.GetSink()->ProcessWebPayRequest(pWebSoap->ip,szAccount,nKindID,nServerID);
	}
	return SOAP_OK;
}
...
//启动服务
bool CProcessWebService::StartService(unsigned short usPort,int nThreadNum)
{
...
{ 
	try
	{
		...
		fd = fopen(szPath, "rb"); //open WSDL file to copy
		if (!fd) throw 0;
