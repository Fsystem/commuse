...
//std::vector<std::string> GUARD::m_stack;
//
//GUARD::GUARD(const char* szLog)
//{
//	m_stack.push_back(szLog);
////	MessageBoxA(NULL, szLog, NULL, NULL);
//}
//
//GUARD::~GUARD()
//{
//	m_stack.pop_back();
////	MessageBoxA(NULL, "popback", NULL, NULL);
//}
...
{
    return timeGetTime();
    //因为下面的时间计算需要刷新，以及相对精度不高，因此不用，直接使用winmm的timeGetTime()
	//return (DWORD)(gTrueTime.HQ_Timer( CFuncPerformanceLog::TIMER_GETABSOLUTETIME )*1000);
}
...
void appUnwindf( const CHAR* Fmt, ... )
{
	...
	va_list	va;
	va_start( va, Fmt );
	wvnsprintfA(TempStr, sizeof(TempStr)-1, Fmt, va );
	va_end( va );
	...
	{	
		...
		strncat( szAppName, ".xml", freesize-1 );
	}
